...
def generate_questions(context):
  tokenizer = AutoTokenizer.from_pretrained("../questions_generation/training_result/qg_small")
  model = AutoModelForSeq2SeqLM.from_pretrained("../questions_generation/training_result/qg_small")
  model.eval()

  text = "question generation: " + context
...
def generate_answer(question, context):
  question_answerer = pipeline("question-answering", model="../question_answering/training_result/qa", device="cuda")
  qa_result = question_answerer(input={'question': question, 'context': context})
  return qa_result

def generate_options(question, answer, model_name_or_path):
...
def streamlit_ui():
  ...
  if prompt := st.chat_input("what's your message?"):
    ...
    if 'Q1' in prompt:
      st.session_state.user_response.append(prompt.split(': ')[1])

      question = st.session_state.questions_list[1]
      answer = generate_answer(question, st.session_state.context)
      if len(answer) != 0:
          answer = answer['answer']
      st.session_state.answers.append(answer)
      options = generate_options(question, answer, '../options_generation/training_result/og')
      response = f"Question 2：\n{question}\n\nOptions:\n\n (1) {options[0]}\n\n (2) {options[1]}\n\n (3) {answer}\n\n (4) {options[2]}"
    elif 'Q2' in prompt:
      st.session_state.user_response.append(prompt.split(': ')[1])

      question = st.session_state.questions_list[2]
      answer = generate_answer(question, st.session_state.context)
      if len(answer) != 0:
          answer = answer['answer']
      st.session_state.answers.append(answer)
      options = generate_options(question, answer, '../options_generation/training_result/og')
      response = f"Question 3：\n{question}\n\nOptions:\n\n (1) {answer}\n\n (2) {options[0]}\n\n (3) {options[1]}\n\n (4) {options[2]}"
    elif 'Q3' in prompt:
      st.session_state.user_response.append(prompt.split(': ')[1])
      correct_count = 0
      corrected_response = ""
      if st.session_state.user_response[0] == '2':
        correct_count += 1
      else:
        corrected_response += 'Q1: (2)' + st.session_state.answers[0] + '\n\n'
      if st.session_state.user_response[1] == '3':
        correct_count += 1
      else:
        corrected_response += 'Q2: (3)' + st.session_state.answers[1] + '\n\n'
      if st.session_state.user_response[2] == '1':
        correct_count += 1
      else:
        corrected_response += 'Q3: (1)' + st.session_state.answers[2] + '\n\n'

      response = f"答題狀況：\n\n 答對：{correct_count} 題\n\n 答錯：{3-correct_count} 題\n\n ====== << WA 的 正確答案 >> ====== \n\n {corrected_response} \n\n 接下來你可以與機器人進行問答："
    elif 'context' in prompt:
      st.session_state.context = prompt
      st.session_state.questions_list = generate_questions(st.session_state.context)
      question = st.session_state.questions_list[0]
      ...
      response = f"Question 1：\n{question}\n\nOptions:\n\n (1) {options[0]}\n\n (2) {answer}\n\n (3) {options[1]}\n\n (4) {options[2]}"
       ...
       completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": f"你是一位人工智慧助理，請你依據文章內容回答用戶的問題，文章內容如下：{st.session_state.context}"},
                {"role": "user", "content": prompt},
            ]
        )
       response = '根據文章內容我們可以知道' + completion.choices[0].message.content
